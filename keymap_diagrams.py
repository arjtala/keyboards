...
keymap_filename = "layouts/arj_ferris/keymap.c"
svg_filename = "images/keymap_diagram.svg"
...
layer_title_spacing = 20 # vertical spacing between layer title and its layer diagram
...
annotation_keycodes = { # keycodes with annotations
    "layer": {"keycodes": ["NUM", "SYM"],
              "label": "layer"},
    "one-shot": {"keycodes": ["OSM(MOD_MEH)", "OSM(MOD_LCTL)", "OS_SHFT", "OS_CTRL", "OS_ALT", "OS_CMD"],
                 "label": "one-shot"},
    held_css_class: {"keycodes": ["LT(1,TAB)", "LT(2,ENT)"], # special case, to provide label for held layer keys
                     "label": "held"},
}
...
key_layers = {}
layer_order = []
row_regexp = re.compile(r"\((?:[^)(]|\((?:[^)(]|\([^)(]*\))*\))*\)", re.M)
key_row_matches = row_regexp.finditer(keymap_raw)
led_row_matches = row_regexp.finditer(ledmap_raw)
for row, key_row_match in enumerate(key_row_matches):
    key_layers.update({layer_num_to_name[row]: key_row_match.group(0)[1:-1].replace("KC_", "").split(", ")})
    layer_order.append(layer_num_to_name[row])
...
print('\n{star}\n{mod}\n{star}'.format(star=''.join(['*']*10), mod=layer_order))
